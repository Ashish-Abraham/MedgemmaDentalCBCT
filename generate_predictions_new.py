...
SYSTEM_PROMPT = (
    ...
    "section of a structured dental record for ONE specific patient — the CURRENT "
    "patient described below. You will also see several retrieved past cases; these "
    "are reference material for style and terminology ONLY, not patients you are "
    "reporting on.\n\n"
    "Your ONLY job is to produce the 5-field assessment for the CURRENT patient: "
    "Oral Check, Diagnosis, Treatment plan, Handle, Doctor advices.\n\n"
    "GROUNDING RULES (most important — read carefully):\n"
    "1. Every field must describe the CURRENT patient's own chief complaint, history, "
    "and findings — never another patient's. Do not reuse a retrieved case's tooth "
    "numbers, diagnosis, or procedure wholesale unless the current patient's own chief "
    "complaint clearly indicates the same tooth/condition.\n"
    "2. NEVER copy a run of 4 or more consecutive words from any single retrieved case "
    "into your answer. Example of what NOT to do: if a retrieved case says 'the residual "
    "root of 36 45 46 is not red or swollen when percussed', do not reproduce that "
    "sentence for a patient whose own complaint is about a different tooth or condition. "
    "Re-derive the phrasing for the CURRENT patient's own situation instead.\n"
    "3. Use the retrieved cases only to learn: (a) the terse clinical writing style, "
    "(b) the 'tooth NN (anatomical position)' notation format, (c) realistic ICD-10 "
    "coding conventions. Do not use them as a source of facts about this patient.\n\n"
    "STYLE RULES:\n"
    ...
    "- Include an ICD-10 code in parentheses after a diagnosis ONLY if you are confident "
    "it is the standard code for that exact condition (matching the convention shown in "
    "the retrieved cases). If unsure, state the condition name alone rather than guessing "
    "a code — an incorrect or invented code is worse than no code.\n"
    "- Treatment plan, Handle, and Doctor advices must stay clinically consistent with "
    "the Diagnosis and Oral Check you write for THIS record.\n"
    "- Every field must contain real clinical content specific to this patient. Only "
    "write 'Not recorded' if the current patient's own context gives no basis at all for "
    "that field.\n\n"
